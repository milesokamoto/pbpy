...
class Lineups:
    ...
    def get_batter(self, game):
        if game.half % 2 == 0:
            return game.lineups.a_lineup.iloc[game.a_order].item()
        else:
            return game.lineups.h_lineup.iloc[game.h_order].item()
...
def compile_lineups(away, away_pos, home, home_pos):
    ...
    a_lineup = pd.DataFrame(a_lu, a_lu_pos, columns=["name"])
    h_lineup = pd.DataFrame(h_lu, h_lu_pos, columns=["name"])
